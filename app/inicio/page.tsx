...
function RestaurantAccessCard({
 ...
 const reportesHref = getScopedHref('/admin/analytics', restaurant.id);
const menuHref = getScopedHref('/admin/productos', restaurant.id);
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <Link
  href={menuHref}
  className="rounded-2xl border border-orange-300 bg-orange-50 px-4 py-4 text-sm font-semibold text-orange-900 hover:bg-orange-100"
>
  🍔 Menú / productos
  <span className="mt-1 block text-xs font-normal text-orange-800">
    Productos, precios, disponibilidad, stock y visibilidad de esta sucursal.
  </span>
</Link>
  ...
  <Link
    href={reportesHref}
    className="rounded-2xl border border-violet-300 bg-violet-50 px-4 py-4 text-sm font-semibold text-violet-900 hover:bg-violet-100"
  >
    📊 Reportes
    <span className="mt-1 block text-xs font-normal text-violet-800">
      Reportes operativos, stock por sucursal y analytics según el plan.
    </span>
  </Link>
) : (
  <Link
  ...
  >
    ...
    <span className="mt-1 block text-xs font-normal text-blue-800">
      Reportes operativos disponibles desde Pro.
    </span>
  </Link>
)}
...
const canUseReports = hasOperationalManagement || canUseAnalytics;
const hasWhatsappDelivery = addons.whatsapp_delivery === true;
  ...
  const links: QuickLink[] = [
    {
      ...
      description:
        'Plan, add-ons, configuración del tenant y configuración de sucursales.',
      badge: 'Base',
    },
    {
  ...
  links.push({
    href: canUseReports ? '/admin/analytics' : '/#precios',
    title: '📊 Reportes',
    description: canUseReports
      ? canUseAnalytics
        ? 'Reportes operativos, control de stock, stock inteligente, KPIs, marcas, canales, productos y exportaciones.'
        : 'Reportes operativos y control de stock por sucursal. Analytics avanzado disponible en Intelligence.'
      : 'Reportes operativos y control de stock disponibles desde Pro.',
    badge: canUseAnalytics ? 'Intelligence' : canUseReports ? 'Pro' : 'Pro',
    tone: canUseAnalytics
      ? 'intelligence'
      : canUseReports
      ? 'pro'
      : 'pro',
  ...
  return links;
}, [canUseAnalytics, canUseReports, hasWhatsappDelivery]);
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-4">
            {[
              ...
              {
  step: '4',
  title: 'Operación y reportes separados',
  desc: 'Pedidos, mesas, stock, productos y reportes se filtran por sucursal.',
},
            ].map((item) => (
              <div
                key={item.step}
